...
const Home = () => {
  ...
  return (
    <div className="min-h-screen">
      {/* Enhanced Navigation */}
      <nav className="fixed top-16 left-0 right-0 z-40 bg-black/30 backdrop-blur-xl border-b border-purple-500/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-center space-x-4 sm:space-x-8 py-3 sm:py-4 overflow-x-auto">
            {[
              { id: 'home', label: 'Home' },
              { id: 'about', label: 'About Us' },
              { id: 'features', label: 'Features' },
              { id: 'roadmap', label: 'Roadmap' },
              { id: 'live-spaces', label: 'Live Spaces' },
      ...
      <section id="home" className="relative py-20 sm:py-32 px-4 sm:px-6 lg:px-8 mt-16 overflow-hidden">
        <div className="max-w-7xl mx-auto">
          <div className="text-center">
            ...
            >
              <div className="bg-gradient-to-r from-purple-900/30 to-pink-900/30 backdrop-blur-lg border border-purple-500/30 rounded-3xl p-6 sm:p-8 shadow-2xl">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-lg sm:text-xl font-semibold text-white flex items-center">
                    <Play className="h-5 w-5 mr-2 text-purple-400" />
                    Featured: "Blockchain Revolution Explained"
                  </h3>
                ...
                <div className="bg-black/30 rounded-2xl p-4 sm:p-6">
                  <audio controls className="w-full mb-4">
                    <source src="/demo-blockchain-audio.mp3" type="audio/mpeg" />
                    Your browser does not support the audio element.
                  </audio>
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-2 sm:space-y-0">
                    <p className="text-sm text-gray-400">
                      AI-Generated Demo: Understanding Blockchain Technology
                    </p>
                    <div className="flex items-center space-x-4 text-xs text-gray-500">
                      <span>Duration: 3:45</span>
                      <span>Quality: HD Audio</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {/* Enhanced Stats Section - Realistic for startup */}
      <section className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8 bg-black/20">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-8">
            <h3 className="text-2xl font-bold text-white mb-2">Early Traction</h3>
            <p className="text-gray-400">Growing community of Web3 audio enthusiasts</p>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-8">
            {[
              { icon: Headphones, value: stats.listeners, label: 'Early Listeners', color: 'purple', growth: '+15% weekly' },
              { icon: Users, value: stats.creators, label: 'Pioneer Creators', color: 'pink', growth: '+8 this month' },
              { icon: TrendingUp, value: stats.nftsMinted, label: 'NFTs Minted', color: 'cyan', growth: '+12 this week' },
              { icon: Mic, value: stats.liveSpaces, label: 'Live Sessions', color: 'green', growth: '+3 weekly' }
            ].map((stat, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 20 }}
              ...
              >
                <div className={`bg-gradient-to-br from-${stat.color}-600/20 to-${stat.color}-800/20 backdrop-blur-sm border border-${stat.color}-500/30 rounded-2xl p-4 sm:p-6 hover:scale-105 transition-transform`}>
                  <stat.icon className={`h-8 sm:h-12 w-8 sm:w-12 text-${stat.color}-400 mx-auto mb-3 sm:mb-4`} />
